...
def calculate_tfidf(desc_list):
    tfidf_scores = []
    preprocessed_desc_list = [description_preprocessing(desc, stop_words) for desc in desc_list]
...
def text_embedding(tfidf_scores):
    for i, tfidf in enumerate(tfidf_scores):
        total = np.zeros(ft.get_dimension())
        total_score = 0
        print(f"TF-IDF scores for description {i+1}:")
        ...
        if(i == 0):
            ...
            print(cosine_similarity(first_desc_embedded, total))
